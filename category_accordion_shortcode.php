// ==========================================
// Category Accordion Dropdowns - SHORTCODE VERSION
// Use shortcode: [category_accordion]
// ==========================================

if (!function_exists('wf_category_accordion_shortcode')) {
    add_shortcode('category_accordion', 'wf_category_accordion_shortcode');

    function wf_category_accordion_shortcode($atts) {
        // Get current category
        if (is_product_category()) {
            $term = get_queried_object();
            $term_id = $term->term_id;
        } else {
            // Allow manual category ID via shortcode attribute
            $atts = shortcode_atts(array('category_id' => 0), $atts);
            $term_id = intval($atts['category_id']);
            if (!$term_id) {
                return '<p>Please specify a category_id in the shortcode.</p>';
            }
        }
